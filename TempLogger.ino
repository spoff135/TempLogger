...
#define WEB_TEMP1 "58aeeb1b7625420815099fbb"
#define WEB_TEMP2 "58aeeb1e7625420814a17b92"
...
byte addrT1[8] = {0x28,0x00,0xBC,0xF2,0x08,0x00,0x00,0x23};
byte addrT2[8] = {0x28,0x0D,0x8E,0xF2,0x08,0x00,0x00,0x6E};

int refreshRateMS = 60000;  
float T1fahrenheit, T2fahrenheit;
...
void loop(void) {
  ...
  //Read sensor T1
  T1fahrenheit = ReadTempSensor(addrT1);
  temperatureStr = String(T1fahrenheit); // store temp in "temperature" string
  Particle.publish("T1", temperatureStr, PRIVATE); // publish to cloud
  delay(1000);

  //Read sensor T2
  T2fahrenheit = ReadTempSensor(addrT2);
  temperatureStr = String(T2fahrenheit); // store temp in "temperature" string
  Particle.publish("T2", temperatureStr, PRIVATE); // publish to cloud
...
//------------------------------------------------------------------------
float ReadTempSensor(byte addr[8]){
...
*/
  ...
  ds.select(addrT1);          // now select the device we just found
...
void PostData(){
    ...
    request.body = "[";
    request.body += "{\"variable\":\""WEB_TEMP1"\", \"value\": "+String(T1fahrenheit,3)+" }";
    request.body += ", { \"variable\":\""WEB_TEMP2"\", \"value\": "+String(T2fahrenheit,3)+" }";
    request.body += "]";
